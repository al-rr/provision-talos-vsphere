...
variable "packer_host" {
  type        = string
  description = "Host onde o packer está sendo executado"
  default     = env("PACKER_HOST")
}
variable "packer_ks_port" {
  type        = number
  description = "Porta usada pelo Kickstart de instalação dos sistemas operacionais ol/rhel/centos"
  default     = env("PACKER_KS_PORT")
}

# Enviroment
variable "env" {
  type = string
  description = "The environment to use for the build. One of `dev`, `test`, or `prod`."
  default = "dev"
}
...
variable "vm_name" {
  type        = string
  description = "Virtual Machina Name"
}
...
variable "template_name" {
  type        = string
  description = "Name of template created and exported to content library."
}
...
variable "build_user_gecos" {
  type        = string
  description = "The name of user to login to the guest operating system."
}
