...
network --hostname=teste.al.rr.leg.br --bootproto=dhcp --onboot=on --nameserver=192.168.0.139 --activate
...
user --gecos="${build_user_gecos}" --name=${build_username} --iscrypted --password=${build_password_encrypted} --groups=wheel
...
# dnf makecache
dnf install -y sudo open-vm-tools net-tools perl vim-minimal
