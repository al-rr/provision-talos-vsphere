#!/usr/bin/env bash
# Compatibility wrapper: delegates HAProxy hardening lifecycle to infra-gitops module.

set -euo pipefail

usage() {
  cat <<'EOF_USAGE'
Usage: hardening.sh [ha-proxy hardening options]
...
All other arguments are forwarded to infra-gitops/scripts/ha-proxy/hardening.sh.
EOF_USAGE
}
...
TARGET_SCRIPT="${INFRA_GITOPS_ROOT}/scripts/ha-proxy/hardening.sh"
...
if [[ "${SHOW_VALUES}" == "true" ]]; then
  ...
  echo "[INFO] Resolved hardening target:"
