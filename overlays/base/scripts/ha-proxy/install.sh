#!/usr/bin/env bash
# @file install.sh
# @brief Install and start HAProxy on a local or remote Linux host.
# @description
#   Installs HAProxy with apt/dnf/yum, enables the service, and ensures it is running.
...
#
# @example
#   ./overlays/base/scripts/ha-proxy/install.sh --env=prod
# @example
#   ./overlays/base/scripts/ha-proxy/install.sh --env=lab --host=172.17.20.91 --user=rocky --ssh-key=/home/vagrant/.ssh/id_ed25519
...
install_haproxy() {
  local pkg_manager="$1"
  log_info "Installing HAProxy on ${TARGET_LABEL} using ${pkg_manager}"

  case "${pkg_manager}" in
    apt)
      run_target "$(as_root_cmd "apt-get update")"
      run_target "$(as_root_cmd "DEBIAN_FRONTEND=noninteractive apt-get install -y -o Dpkg::Options::=--force-confold haproxy policycoreutils-python-utils")"
      ;;
    dnf)
      run_target "$(as_root_cmd "dnf install -y haproxy policycoreutils-python-utils")"
      ;;
    yum)
      run_target "$(as_root_cmd "yum install -y haproxy policycoreutils-python-utils")"
      ;;
    *)
...
enable_service() {
  log_info "Ensuring service ${HAPROXY_SERVICE_NAME_RESOLVED} is enabled and running on ${TARGET_LABEL}"
  run_target "$(as_root_cmd "systemctl enable --now ${HAPROXY_SERVICE_NAME_RESOLVED}")"
}
...
validate_service_status() {
  ...
  status="$(capture_target "$(as_root_cmd "systemctl is-active ${HAPROXY_SERVICE_NAME_RESOLVED}")" | tr -d '[:space:]' || true)"
  [[ "${status}" == "active" ]] || die "Service ${HAPROXY_SERVICE_NAME_RESOLVED} is not active on ${TARGET_LABEL}."
  log_info "Service ${HAPROXY_SERVICE_NAME_RESOLVED} is active."
}
...
main() {
  ...
  [[ -n "${HAPROXY_SERVICE_NAME_RESOLVED}" ]] || die "HAPROXY_SERVICE_NAME is empty."

  detect_target_privilege
  install_haproxy "$(detect_package_manager)"
  enable_service
  validate_service_status

  log_info "HAProxy install completed for ${TARGET_LABEL}."
}
