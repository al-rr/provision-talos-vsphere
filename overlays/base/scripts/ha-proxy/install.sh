#!/usr/bin/env bash
# Compatibility wrapper: delegates HAProxy install lifecycle to infra-gitops module.

set -euo pipefail

usage() {
  cat <<'EOF_USAGE'
Usage: install.sh [ha-proxy install options]
...
All other arguments are forwarded to infra-gitops/scripts/ha-proxy/install.sh.
EOF_USAGE
}
...
TARGET_SCRIPT="${INFRA_GITOPS_ROOT}/scripts/ha-proxy/install.sh"
