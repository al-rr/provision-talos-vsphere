...
# @describe Export Ansible variables from overlay vars.
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  echo "Usage: OVERLAY_ENV=prod source ./load-ansible-vars.sh"
  exit 1
fi
...
echo "[OK] Ansible variables exported for overlay ${OVERLAY_ENV:-prod}"
